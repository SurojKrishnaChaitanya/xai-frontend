...
//   // Sync GeoJSON Heatmap Source & MapLibre Markers
//   useEffect(() => {
...
//       if (map.getSource('cities-heat-source')) {
//         map.getSource('cities-heat-source').setData(geojsonData);
//       } else {
//         map.addSource('cities-heat-source', {
//           type: 'geojson',
//           data: geojsonData,
//         });
...
//               ['heatmap-density'],
//               0, 'rgba(0, 0, 0, 0)',
//               0.2, 'rgba(16, 185, 129, 0.75)',
//               0.5, 'rgba(245, 158, 11, 0.85)',
//               0.8, 'rgba(239, 68, 68, 0.95)',
//               1.0, 'rgba(159, 18, 57, 1)'
//             ],
//             'heatmap-radius': ['interpolate', ['linear'], ['zoom'], 2, 40, 6, 80, 10, 120],
...
//         const color = city.riskScore > 70 ? '#e11d48' : city.riskScore > 40 ? '#d97706' : '#10b981';
...
//         iconSvg.innerHTML = `
//           <svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="${color}" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="filter: drop-shadow(0px 4px 6px rgba(0,0,0,0.3));">
...
//     }
//   }, [cities, selectedCity, showHeatmap]);

//   // Toggle Heatmap Layer Opacity
//   useEffect(() => {
//     const map = mapInstanceRef.current;
//     if (map && map.getLayer('risk-heatmap-layer')) {
//       map.setPaintProperty('risk-heatmap-layer', 'heatmap-opacity', showHeatmap ? 0.85 : 0);
//     }
//   }, [showHeatmap]);
...
//       {/* Header Controls */}
//       <div className="absolute top-4 left-4 right-4 z-[10] pointer-events-none flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
...
//           </div>
//           <div>
//             <h1 className="text-sm font-bold text-slate-900">National Risk & Weather Radar</h1>
//             <p className="text-[11px] text-slate-500">Live target zone heat intensity & node analytics</p>
//           </div>
//         </div>
...
//           <button
//             onClick={() => mapInstanceRef.current?.flyTo({ center: INDIA_CENTER, zoom: INDIA_ZOOM })}
...
//       {/* Heatmap Legend */}
//       <div className="absolute top-20 right-4 z-[10] bg-white/95 backdrop-blur-md px-3.5 py-2.5 rounded-xl border border-slate-200/90 shadow-lg space-y-1.5 text-xs pointer-events-auto">
//         <div className="flex items-center gap-1.5 text-[11px] font-bold text-slate-700">
//           <Layers className="w-3.5 h-3.5 text-blue-600" />
//           <span>Risk Heat Intensity</span>
//         </div>
//         <div className="h-2 w-36 rounded-full bg-gradient-to-r from-emerald-400 via-amber-400 to-rose-600" />
...
//       {/* Active City Overlay Card */}
//       {selectedCity && (
//         <div className="absolute bottom-6 left-6 z-[10] w-80 bg-white/95 backdrop-blur-md p-5 rounded-2xl border border-slate-200 shadow-xl space-y-3">
//           <div className="flex justify-between items-start border-b border-slate-100 pb-2">
//             <div>
//               <span className="text-[10px] font-bold text-blue-600 uppercase tracking-wider">Active Target Zone</span>
//               <h2 className="text-lg font-extrabold text-slate-900">{selectedCity.name}</h2>
//               <p className="text-xs text-slate-500">{selectedCity.state}</p>
//             </div>
//             <div className="text-right">
//               <span className="text-2xl font-black text-rose-600 font-mono">{selectedCity.riskScore}</span>
//               <span className="text-[10px] text-slate-400 block font-semibold">/ 100 Risk</span>
//             </div>
//           </div>
...
//               <span className="font-mono text-slate-700">{selectedCity.lat?.toFixed(4)}° N</span>
//             </div>
//             <div className="p-2 bg-slate-50 rounded-lg border border-slate-100">
//               <span className="text-[10px] text-slate-400 block font-normal">Longitude</span>
//               <span className="font-mono text-slate-700">{selectedCity.lng?.toFixed(4)}° E</span>
//             </div>
//           </div>
//         </div>
//       )}
