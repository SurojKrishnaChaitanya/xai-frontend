...
export default function AppLayout() {
  ...
  const connectWebSocket = useWeatherStore((state) => state.connectWebSocket);
  const disconnectWebSocket = useWeatherStore((state) => state.disconnectWebSocket);
  const streamPaused = useWeatherStore((state) => state.streamPaused);

  useEffect(() => {
    if (!selectedRegion || streamPaused) {
      disconnectWebSocket();
      return;
    }
    connectWebSocket();
    return () => disconnectWebSocket();
  }, [selectedRegion?.id, streamPaused]);
